#!/usr/bin/env python3
"""Validate fail-closed metadata object key types for memory-item writes."""
...
def memory_record(*, metadata: dict[Any, Any]) -> MemoryItemRecord:
    return MemoryItemRecord(
        ...
        agent_id=None,
        session_id=None,
        content="metadata key type validation",
        source_run_id=None,
        metadata=metadata,
        expires_at=None,
...
def check_non_string_keys_fail_before_write() -> list[str]:
    errors: list[str] = []
    ...
    expected_message = (
        "Memory item memory-1 write requires metadata object keys to be strings."
    )
    for label, metadata in cases:
        ...
        try:
            ...
            if str(exc) != expected_message:
                errors.append(f"{label} must return the stable key type error")
        else:
            errors.append(f"memory-item writes must reject {label}")
        if database.transaction_calls != 0:
            errors.append(f"{label} must fail before the transaction")
        if database.execute_parameters is not None:
            errors.append(f"{label} must fail before cursor execution")
    return errors


def check_string_keys_still_write() -> list[str]:
    errors: list[str] = []
    for label, metadata in (
        ...
        try:
            ...
            errors.append(f"valid {label} keys must be accepted")
            continue
        if persisted != record:
            errors.append(f"valid {label} keys must preserve the record")
        if database.transaction_calls != 1:
            errors.append(f"valid {label} keys must reach the transaction")
        if database.execute_parameters is None:
            errors.append(f"valid {label} keys must reach cursor execution")
        else:
            serialized_metadata = database.execute_parameters[7]
            if not isinstance(serialized_metadata, str):
                errors.append(f"valid {label} keys must be serialized")
            elif json.loads(serialized_metadata) != metadata:
                errors.append(f"valid {label} keys must preserve their JSON value")
    return errors
...
def check_key_validation_precedes_serialization() -> list[str]:
    ...
    try:
        ...
        if str(exc) != expected_message:
            errors.append("metadata key validation must precede JSON serialization")
    else:
        errors.append("invalid metadata keys must fail before serialization")
    if database.transaction_calls != 0:
        errors.append("metadata key validation must precede the transaction")
    return errors
...
def check_source_and_gate() -> list[str]:
    ...
    key_check = "if any(not isinstance(key, str) for key in value):"
    serialization_call = "json.dumps(record.metadata"
    if key_check not in serializer_source:
        errors.append("memory-item writes must require string metadata object keys")
    elif serializer_source.index(key_check) > serializer_source.index(
        serialization_call
    ):
        errors.append("metadata key validation must precede serialization")
    check_name = "check_phase6_memory_item_write_metadata_key_type_fail_closed.py"
...
def main() -> int:
    errors = [
        *check_non_string_keys_fail_before_write(),
        *check_string_keys_still_write(),
    ...
    if errors:
        for error in errors:
            print(
                f"[phase6-memory-item-write-metadata-key-type] {error}",
                file=sys.stderr,
            )
        return 1
    print("[phase6-memory-item-write-metadata-key-type] passed")
    return 0
