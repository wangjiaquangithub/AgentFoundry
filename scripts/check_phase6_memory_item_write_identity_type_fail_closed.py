#!/usr/bin/env python3
"""Validate fail-closed required identity types for memory-item writes."""
...
def check_malformed_required_identity_fails_before_write() -> list[str]:
    errors: list[str] = []
    cases = (
        ("item id None", {"id": None}),
        ("item id integer", {"id": 7}),
        ("tenant id None", {"tenant_id": None}),
        ("tenant id integer", {"tenant_id": 7}),
        ("user id None", {"user_id": None}),
        ("user id integer", {"user_id": 7}),
    )
    expected_messages = {
        "id": "Memory item write requires a non-blank id.",
        "tenant_id": "Memory item write requires a non-blank tenant id.",
        "user_id": "Memory item write requires a non-blank user id.",
    }
    for label, overrides in cases:
        record = memory_record(**overrides)
        database = FakePostgresDatabase(memory_row(record))
        field_name = next(iter(overrides))
        try:
            PostgresMemoryItemWriteRepository(database).append_memory_item(record)
        except ValueError as exc:
            if str(exc) != expected_messages[field_name]:
...
def check_string_required_identity_is_accepted() -> list[str]:
    ...
    try:
        ...
        return ["string required identities must remain accepted"]
    errors: list[str] = []
    if persisted != record:
        errors.append("identity type validation must preserve the record")
    if database.transaction_calls != 1:
        errors.append("valid identity types must reach the write transaction")
    return errors
...
def check_source_and_gate() -> list[str]:
    ...
    if "isinstance(value, str)" not in validator_source:
        errors.append("memory-item write identities must require strings")
    check_name = "check_phase6_memory_item_write_identity_type_fail_closed.py"
    if check_name not in gate_source:
        errors.append("Phase 6 backend gate must run the identity-type check")
    return errors


def main() -> int:
    errors = [
        *check_malformed_required_identity_fails_before_write(),
        *check_string_required_identity_is_accepted(),
        *check_source_and_gate(),
    ]
    if errors:
        for error in errors:
            print(
                f"[phase6-memory-item-write-identity-type] {error}",
                file=sys.stderr,
            )
        return 1
    print("[phase6-memory-item-write-identity-type] passed")
    return 0
