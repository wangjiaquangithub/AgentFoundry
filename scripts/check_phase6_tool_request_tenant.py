#!/usr/bin/env python3
"""Validate Tool routes against the authenticated request tenant."""
...
from api.tools import _request_tenant  # noqa: E402
...
def assert_request_identity_tenant_is_authoritative() -> None:
    ...
    try:
        ...
        raise AssertionError("cross-tenant Tool runtime must be rejected")
...
def assert_route_wiring_validates_runtime_tenant() -> None:
    ...
    assert source.count("tenant=runtime_selection[\"tenant\"],") == 3
    assert source.count("tenant_hint_from_user_id=deps.tenant_hint_from_user_id,") >= 4
    assert "scripts/check_phase6_tool_request_tenant.py" in gate_source
...
def main() -> int:
    ...
    print("[phase6-tool-request-tenant] passed")
    return 0
