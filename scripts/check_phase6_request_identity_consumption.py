...
def check_knowledge_consumption() -> list[str]:
    ...
    if source.count("get_request_identity(request)") != 6:
        errors.append(
            "knowledge.py must resolve canonical identity at all 6 identity consumption points"
        )
    return errors
...
def check_workflows_consumption() -> list[str]:
    ...
    if source.count("get_request_identity(request)") != 10:
        errors.append(
            "workflows.py must preserve all 10 canonical identity consumption points"
        )
    return errors
...
def check_agent_runtime_consumption() -> list[str]:
    ...
    if source.count("get_request_identity(request)") != 3:
        errors.append(
            "agent_runtime.py must resolve canonical identity for execution and mutation routes"
        )
