#!/usr/bin/env python3
"""Validate authenticated actor audit evidence for platform Agent publishes."""
...
def publish_payload() -> SimpleNamespace:
...
def check_publish_audit_contract() -> list[str]:
    audit_writer = AuditEventWriter()
    service = build_service(audit_writer=audit_writer)
    response = service.publish_agent_response_payload(
        publish_payload(),
        "acme:admin",
        tenant="acme",
    )
    ...
    errors: list[str] = []
    published_agent = response["agent"]
    published_agent_id = published_agent["id"]
    if published_agent["tenant"] != "acme":
        errors.append("Agent publish must persist within the request tenant")
    if published_agent["created_by"] != "acme:admin":
        errors.append("published Agent created_by must equal the authenticated actor")
    acme_agent_ids = {agent["id"] for agent in service.list_agents(tenant="acme")}
    if acme_agent_ids != {"agent_acme", published_agent_id}:
        errors.append("Agent publish must add exactly one request-tenant Agent")
    globex_agent_ids = [
        agent["id"] for agent in service.list_agents(tenant="globex")
    ]
    if globex_agent_ids != ["agent_globex"]:
        errors.append("Agent publish must preserve other tenant records")
    if len(audit_writer.records) != 1:
        return errors + ["Agent publish must append exactly one audit event"]
    ...
    if event.payload != expected_payload:
        errors.append("audit payload must contain the normalized Agent publish evidence")
    return errors
...
def check_audit_fail_closed() -> list[str]:
    ...
    try:
        failing_service.publish_agent_response_payload(
            publish_payload(),
            "acme:admin",
            tenant="acme",
        )
    ...
    try:
        build_service(audit_writer=blank_id_writer).publish_agent_response_payload(
            publish_payload(),
            "acme:admin",
            tenant="acme",
        )
...
def check_route_composition_and_gate() -> list[str]:
    ...
    publish_request_start = route_source.index(
    ...
    if not identity_start < tenant_start < publish_request_start < mutation_start:
        errors.append("Agent publish must resolve identity and tenant before mutation")
    if "header_user_id=identity.user_id," not in route_source:
        errors.append("Agent publish request must receive the authenticated request actor")
    mutation_source = route_source[mutation_start:]
    if "payload,\n                user_id,\n                tenant=tenant," not in mutation_source:
        errors.append("Agent publish mutation must receive the resolved actor and tenant")
    ...
    if "check_phase6_agent_publish_actor_audit.py" not in gate_source:
        errors.append("Phase 6 backend gate must run the Agent publish audit check")
    return errors


def main() -> int:
    errors = check_publish_audit_contract()
    errors += check_audit_fail_closed()
    errors += check_route_composition_and_gate()
    if errors:
        for error in errors:
            print(f"[phase6-agent-publish-actor-audit] {error}", file=sys.stderr)
        return 1
    print("[phase6-agent-publish-actor-audit] passed")
    return 0
