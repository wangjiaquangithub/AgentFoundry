...
Seeds AgentFoundry development fixture data into the configured database.
  ...
  postgresql://*|postgres://*)
    if command -v uv >/dev/null 2>&1; then
      exec uv run --with "psycopg[binary]" \
        python -c "from backend.persistence.seed import main; main()" \
        --database-url "$DATABASE_URL"
    fi
    exec python3 -c "from backend.persistence.seed import main; main()" \
      --database-url "$DATABASE_URL"
    ;;
  sqlite://*)
    exec python3 -c "from backend.persistence.seed import main; main()" \
