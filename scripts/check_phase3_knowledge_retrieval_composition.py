#!/usr/bin/env python3
"""Check PostgreSQL knowledge retrieval service composition stays explicit."""
...
def _check_factory_contract() -> list[str]:
    ...
    build_service = _function_node(
        tree,
        "build_postgres_knowledge_retrieval_service",
    )
    ...
    if build_service is None:
        errors.append(
            ...
            "build_postgres_knowledge_retrieval_service(database)",
        )
    else:
        for name in (
            ...
            if not _references_name(build_service, name):
                errors.append(
                    "build_postgres_knowledge_retrieval_service must compose "
                    f"{name}",
                )
    ...
    if build_configured is None:
        ...
        if not _calls_name(build_configured, "create_configured_postgres_database"):
            errors.append(
                "configured knowledge retrieval service factory must use the "
        ...
        if not _calls_name(
            ...
            errors.append(
                "configured knowledge retrieval service factory must delegate "
    ...
    for forbidden in ("SQLite", "sqlite", "FastAPI", "APIRouter"):
        if forbidden in source:
            errors.append(
                "knowledge retrieval service factory must stay PostgreSQL-only "
...
def main() -> int:
    ...
    print("Phase 3 PostgreSQL knowledge retrieval composition gate")
    print("- factory: backend/services/composition.py")
    print("- service: PlatformKnowledgeRetrievalService")
    print("- persistence: PostgreSQL read repositories plus event writers")
    print("- API/main direct retrieval wiring: blocked")
    ...
    print("\nOK: PostgreSQL knowledge retrieval composition is explicit.")
    return 0
