#!/usr/bin/env python3
"""Validate authenticated actor audit evidence for Knowledge Base upserts."""
...
from services.knowledge_bases import (  # noqa: E402
...
class KnowledgeBaseWriter:
    ...
    def upsert_knowledge_base(self, record):
...
def command_input() -> KnowledgeBaseApiCommandInput:
    return KnowledgeBaseApiCommandInput(
        id="kb-support",
        tenant_id="acme",
        name="Support Knowledge",
        description="Approved support documentation.",
        status="active",
        embedding_model_config_id="model-config-embedding-primary",
...
def build_service(
    *,
    knowledge_base_writer: KnowledgeBaseWriter | None = None,
    audit_writer: AuditEventWriter | None = None,
) -> PlatformKnowledgeBaseService:
    return PlatformKnowledgeBaseService(
        knowledge_base_writer=knowledge_base_writer or KnowledgeBaseWriter(),
...
def check_upsert_audit_contract() -> list[str]:
    knowledge_base_writer = KnowledgeBaseWriter()
    audit_writer = AuditEventWriter()
    service = build_service(
        knowledge_base_writer=knowledge_base_writer,
        audit_writer=audit_writer,
    )
    response = service.upsert_knowledge_base_from_api(command_input())

    errors: list[str] = []
    if response.tenant_id != "acme":
        errors.append("Knowledge Base upsert must persist within the request tenant")
    if response.id != "kb-support":
        errors.append("Knowledge Base upsert must return the persisted target")
    if len(knowledge_base_writer.records) != 1:
        errors.append("Knowledge Base upsert must persist exactly one record")
    if len(audit_writer.records) != 1:
        return errors + ["Knowledge Base upsert must append exactly one audit event"]
    ...
    if event.payload != expected_payload:
        errors.append("audit payload must contain normalized Knowledge Base evidence")
    return errors
...
def check_audit_fail_closed() -> list[str]:
    ...
    try:
        failing_service.upsert_knowledge_base_from_api(command_input())
    ...
    try:
        build_service(audit_writer=blank_id_writer).upsert_knowledge_base_from_api(
            command_input(),
        )
    except PlatformKnowledgeBaseServiceError as exc:
        if exc.status_code != 500:
...
def check_route_composition_and_gate() -> list[str]:
    ...
    if not tenant_start < identity_start < actor_start < mutation_start:
        errors.append(
            "Knowledge Base upsert must resolve tenant, identity, and actor before mutation",
        )
    mutation_source = route_source[mutation_start:]
    if "tenant_id=tenant_id," not in mutation_source:
        errors.append("Knowledge Base upsert must use the resolved request tenant")
    if "actor_user_id=actor_user_id," not in mutation_source:
        errors.append("Knowledge Base upsert must use the authenticated request actor")
    ...
    if "knowledge_base_writer=PostgresKnowledgeBaseWriteRepository(database)" not in service_source:
        errors.append("production Knowledge Base service must inject the write repository")
    if "audit_event_writer=PostgresAuditEventWriteRepository(database)" not in service_source:
        errors.append("production Knowledge Base service must inject the audit event writer")
    if "check_phase6_knowledge_base_actor_audit.py" not in gate_source:
        errors.append("Phase 6 backend gate must run the Knowledge Base actor audit check")
    return errors
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(f"[phase6-knowledge-base-actor-audit] {error}", file=sys.stderr)
        return 1
    print("[phase6-knowledge-base-actor-audit] passed")
    return 0
