		...
		>
			...
			<div className="grid gap-4 p-4">
				...
				<div className="grid gap-2">
					<label className="text-xs font-medium text-muted-foreground">
						{selectedToolConfig
							...
							: (selectedToolCatalogItem?.input_key ?? t('platform.toolRunner.input'))}
					</label>
					<Input
				...
				<div className="grid gap-2 sm:grid-cols-2">
					...
					>
						<ListChecks
							className={cn(creatingRunApproval === 'tool_run' && 'animate-pulse')}
						/>
						{creatingRunApproval === 'tool_run'
					...
					>
						...
						{runningTool ? t('platform.toolRunner.running') : t('platform.toolRunner.run')}
					</Button>
				</div>
			...
			<div className="border-t bg-slate-50/70 p-4">
				<div className="mb-3 flex items-center gap-2">
					<Code2 className="size-4 text-muted-foreground" />
					<h3 className="text-sm font-semibold">{t('platform.toolRunner.result')}</h3>
				</div>
				{toolRunResult ? (
					<pre className="max-h-96 min-h-56 overflow-auto rounded-lg border bg-background p-4 text-xs leading-5">
						{JSON.stringify(toolRunResult, null, 2)}
					</pre>
				) : (
					<div className="flex min-h-56 items-center rounded-lg border border-dashed bg-background p-6 text-sm text-muted-foreground">
