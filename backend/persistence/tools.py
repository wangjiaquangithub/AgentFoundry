"""SQLite tool governance read repository.

This repository keeps tool catalog and policy reads tenant-scoped while the
...
class SQLiteToolGovernanceReadRepository:
    ...
    def list_tools(
        ...
        *,
        ...
        return [self._tool_from_row(dict(row)) for row in rows]
    ...
    def get_tool(self, *, tenant_id: str, tool_id: str) -> ToolRecord | None:
        ...
        if row is None:
            return None
        return self._tool_from_row(dict(row))
    ...
    def list_policies(self, *, tenant_id: str) -> list[ToolPolicyRecord]:
        ...
        return [self._policy_from_row(dict(row)) for row in rows]
    ...
    def get_policy_for_tool(
        ...
        *,
        ...
        if row is None:
            return None
        return self._policy_from_row(dict(row))
    ...
    def _tool_from_row(self, row: dict[str, Any]) -> ToolRecord:
        schema = self._object_from_json(row["schema"], row["id"], "schema")
        return ToolRecord(
            id=row["id"],
    ...
    def _policy_from_row(self, row: dict[str, Any]) -> ToolPolicyRecord:
        return ToolPolicyRecord(
            ...
            tenant_id=row["tenant_id"],
            tool_id=row["tool_id"],
            allowed_roles=self._string_list_from_json(
                row["allowed_roles"],
                row["id"],
                "allowed_roles",
            ),
            approval_required=bool(row["approval_required"]),
            rate_limit=self._rate_limit_from_json(row["rate_limit"], row["id"]),
